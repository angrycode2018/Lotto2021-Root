...
function createClass(className="") {
    ...
    return class {
        ...
        setDefault({tbody="", tr="", th="", td=""}) {
            this.default = {tbody, tr, th, td}
        }
        setTbody({style="", classList=""}) {
            ...
            if(!this?.Node?.tbody) throw new Error('tbody is not set.');
            isClass('style') && this.Node.tbody.setAttribute('style', style);
            isClass('classlist') && this.Node.tbody.classList.add(...classList.split(/ /g));
            return this;// style
        }
        ...
        lists({tag="", style="", classList=""}) {
            ...
            const all = this.Node.tbody.querySelectorAll(tag);
            if(all.length == 0) {
...
class CSS {
    ...
    #useClass(className="") {
        ...
        // debugger
        return ({tbody="", tr="", th="", td=""}) => {
            if([tbody, tr, th, td].some(v => typeof v !== 'string')) throw new Error('require type of string.');
            instance.setDefault({tbody, tr, th, td});//ถ้า elements ยังไม่ถูกสร้าง จะเก็บ style ไว้ก่อน
            tbody && instance.setTbody({...newObj(tbody)});//tbody ถูกสร้างไว้ก่อนแล้ว
            const trList = Node.tbody.querySelectorAll('tr');
            
            //ถ้าเรียกใช้ TBody.tr() ก่อน TBody.CSS.style() จะเป็นการ apply style หลังจาก element สร้างเสร็จแล้ว
            if(trList.length > 0) {
    ...
    style({tbody="", tr="", th="", td=""}) {
        const applyStyle = this.#useClass('Style')
        const done = applyStyle({tbody, tr, th, td})
        return done && this;
    }
    class({tbody="", tr="", th="", td=""}) {
        const applyClass = this.#useClass('ClassList')
        const done = applyClass({tbody, tr, th, td})
        return done && this;
    }
...
export default class TBody {
    ...
    setTBody() {
        if(this.Node.table.querySelector('tbody')) throw new Error('tbody ถูกสร้างไว้แล้ว ไม่สามารถสร้างซ้ำได้');
        const tbody = node({type: 'tbody'});
        this.Node.set(tbody, 'tbody');
    ...
    tr({ id="", key="", data=['Tom', 30, 'New York'], style={tr:"", td:"", th:""}, classList={tr:"", td:"", th:""} }) {
        ...
        // this.node.querySelector('tbody').appendChild(tr);
        if(!this.Node.tbody) throw new Error('tbody is not found.');
        this.Node.tbody.appendChild(tr)
        return tr;//NOTE - เปลี่ยนจาก this เป็น tr เพื่อให้สามารถเข้าถึงแถวที่สร้างขึ้นได้
    }
}
...
/* โค๊ดเก่า สำคัญ
    // style({tbody="", tr="", th="", td=""}) {
    //     if([tbody, tr, th, td].some(v => typeof v !== 'string')) throw new Error('require type of string.');
    //     this.Style.setDefault({tbody, tr, th, td});//ถ้า elements ยังไม่ถูกสร้าง จะเก็บ style ไว้ก่อน
    //     tbody && this.Style.setTbody(tbody);//tbody ถูกสร้างไว้ก่อนแล้ว
    //     const trList = this.Node.tbody.querySelectorAll('tr');
        
    //     //ถ้าเรียกใช้ TBody.tr() ก่อน TBody.style() จะเป็นการ apply style หลังจาก element สร้างเสร็จแล้ว
    //     if(trList.length > 0) {
    //         tr && this.Style.lists('tr', tr);
    //         th && this.Style.lists('th', th);
    //         td && this.Style.lists('td', td);
    //     }
    //     return this
    // }
*/